...
import 'package:misana_finance_app/core/utils/jwt_utils.dart';
...
class _KycVerificationPageState extends State<KycVerificationPage> {
  ...
  void _showSubmittedDialog() {
    showDialog(
      ...
      builder: (ctx) => _KycSubmittedDialog(userId: _userId),
    );
  }
      ...
      child: Scaffold(
        ...
        body: BlocConsumer<KycBloc, KycState>(
          listener: (context, state) async {
            ...
            if (state.showSubmittedNotification) {
              _showSubmittedDialog();
            }
            if (state.showApprovedNotification) {
              await context.read<AuthCubit>().refreshProfile();
              if (!mounted) return;
              Navigator.of(context).popUntil((route) => route.isFirst);
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text("✅ Uthibitisho umekamilika!"),
                  backgroundColor: Colors.green,
                ),
              );
            }
            if (state.showRejectedNotification) {
              if (!mounted) return;
              Navigator.of(context).popUntil((route) => route.isFirst);
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text("❌ Uthibitisho umekataliwa: ${state.rejectionReason ?? 'Tafadhali jaribu tena'}"),
                            ...
                            child: Column(
                              children: [
                                ...
                                TextFormField(
                                  ...
                                  textCapitalization: TextCapitalization.words,
                                  decoration: const InputDecoration(
                                    labelText: "Mahali pa Kuzaliwa",
                                    prefixIcon: Icon(Icons.location_on_outlined),
                                    hintText: "Mfano: Dar es Salaam",
                                  ),
                                  validator: (v) =>
                                      (v == null || v.trim().isEmpty) ? "Weka mahali pa kuzaliwa" : null,
                                ),
...
class _KycSubmittedDialog extends StatefulWidget {
  final String userId;
  const _KycSubmittedDialog({required this.userId});

  @override
  State<_KycSubmittedDialog> createState() => _KycSubmittedDialogState();
}

class _KycSubmittedDialogState extends State<_KycSubmittedDialog> {
  late StreamSubscription _kycSubscription;

  @override
  void initState() {
    super.initState();
    
    // Listen to KYC bloc for real-time updates
    _kycSubscription = context.read<KycBloc>().stream.listen((state) {
      if (state.isVerified || state.isRejected) {
        if (mounted) {
    ...
    context.read<KycBloc>().add(KycStartPolling(userId: widget.userId));
  }
        ...
        return AlertDialog(
          ...
          title: Row(
            children: [
              ...
              const SizedBox(width: 8),
              const Text('Maombi Yamewasilishwa'),
            ],
          ),
          content: Column(
            ...
            children: [
              const Text(
                'Maombi yako ya uthibitisho yamepokelewa na yanashughulikiwa.',
              ...
              if (state.polling)
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'Inaangalia hali ya uthibitisho...',
              ...
              if (!state.polling)
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'Subiri kidogo tutakuarifu mara uthibitisho utakapokamilika.',
                        style: TextStyle(fontSize: 13, color: Colors.grey[600]),
                      ),
                    ),
                  ],
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'Mchakato unaweza kuchukua dakika 1-5',
