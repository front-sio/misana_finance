...
class AuthRepositoryImpl implements AuthRepository {
  ...
  Future<Map<String, dynamic>> updateProfile(Map<String, dynamic> data) async {
    try {
      developer.log('Updating user profile', name: 'AuthRepo');
      final result = await remote.updateProfile(data);
      developer.log('User profile updated successfully', name: 'AuthRepo');
      return result;
    } catch (e) {
      developer.log('Failed to update profile: $e', name: 'AuthRepo', level: 1000);
      throw Exception(MessageMapper.getAuthErrorMessage(e));
    }
  }
  ...
  Future<Map<String, dynamic>> uploadProfilePicture(File file) async {
    try {
      developer.log('Uploading profile picture', name: 'AuthRepo');
      final result = await remote.uploadProfilePicture(file);
      developer.log('Profile picture uploaded successfully', name: 'AuthRepo');
      return result;
    } catch (e) {
      developer.log('Failed to upload profile picture: $e', name: 'AuthRepo', level: 1000);
      throw Exception(MessageMapper.getAuthErrorMessage(e));
    }
  }
  ...
  Future<void> changePassword(String oldPassword, String newPassword) async {
    try {
      developer.log('Changing password', name: 'AuthRepo');
      await remote.changePassword(oldPassword, newPassword);
      developer.log('Password changed successfully', name: 'AuthRepo');
    } catch (e) {
      developer.log('Failed to change password: $e', name: 'AuthRepo', level: 1000);
      throw Exception(MessageMapper.getAuthErrorMessage(e));
    }
  }
}
